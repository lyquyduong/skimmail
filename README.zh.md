...
- **AI（可选，自带密钥）**——通过 **Claude / OpenAI 兼容 / Ollama** 进行摘要、分类、
  动作抽取、翻译。默认关闭。
- **键盘优先**（Superhuman 风格）+ **专注/处理模式** + **PWA**（可安装、外壳离线可用）
  + **多语言**（English / Tiếng Việt / 中文，深色与浅色主题）。
- **可插拔存储（进阶）**——默认 **SQLite + 文件系统**；可选 **Postgres/MySQL** 与
  **S3 兼容**对象存储（通过 build tag）。
- **同步调度器**（并发 + 速率限制 + 优先级队列）与**安全**层：登录限流 + 锁定、
  CSRF/Origin 校验、SSRF 防护、凭据静态加密、以及审计日志。
...
用 `advanced` profile 测试**无状态栈**（Postgres + S3/MinIO）：

```bash
docker compose --profile advanced up --build
# → skimmail:      http://localhost:8081
# → MinIO 控制台:  http://localhost:9001  (minioadmin / minioadmin)
docker compose --profile advanced down -v   # 停止 + 清空卷
```
...
- **阅读区**——🗄 归档 · 🗑 删除（可 **Undo**） · 💤 稍后 · 📌 置顶 · ✨ 摘要 ·
  ⚡ 动作 · 🌐 翻译 · ✉️ 退订 · ★ VIP · 🔇 静音。
- **AI**——打开 **🤖 AI settings**，启用后选择厂商 + 模型 + 密钥（或使用本地
  **Ollama** 免密钥）。邮件内容会发送到你选择的厂商。
...
| `MAX_CONCURRENT_SYNCS` | `4` | 同时同步的最大邮箱数（调度器） |
...
| `DB_DRIVER` | `sqlite` | `sqlite` \| `postgres` \| `mysql`（进阶） |
| `DATABASE_URL` | – | 当 `DB_DRIVER` ≠ sqlite 时的 DSN |
| `BLOB_STORE` | `fs` | `fs` \| `s3`（进阶） |
| `S3_ENDPOINT` / `S3_BUCKET` / `S3_REGION` / `S3_ACCESS_KEY` / `S3_SECRET_KEY` / `S3_FORCE_PATH_STYLE` | – | S3 兼容对象存储 |

### 进阶：Postgres / MySQL / S3

默认镜像为 SQLite + 本地文件系统。若要在 **Postgres/MySQL** 上运行并使用 **S3 兼容**
对象存储，可使用 Compose 的 **`advanced` profile**（见上），或构建一个编译进相应后端
的自定义镜像：
...
然后按上表设置 `DB_DRIVER` / `DATABASE_URL` / `BLOB_STORE` / `S3_*`（MySQL DSN 需
`multiStatements=true`）。从源码构建的细节见 [`README.dev.md`](README.dev.md)。

---

## 安全

凭据与 AI/OAuth 令牌静态加密（AES-256-GCM）。服务端实施登录限流 + 锁定、CSRF/Origin
